...
namespace LivroDeRegistos_v1
{
    ...
    public class Registo_Livro : MainForm
    {
        ...
        {
            if (IsRegistrationNumberExists(nRegisto))
            {
                MessageBox.Show("Houve uma falha ao registar: este Nº de registo já existe", "Falha", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            try
            {
                int autorID;
            ...
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    for (int i = startNumber; i <= endNumber; i++)
                        if (!usedNumbers.Contains(i)) availableNumbers.Add(i);
